...
class SceneTextDataset(Dataset):
    def __init__(self, root_dir,
        ...
        self._lang_list = ['chinese', 'japanese', 'thai', 'vietnamese', 'classic']
        ...
        for nation in self._lang_list:
            if nation == 'classic':
                # classic일 경우 파일 경로를 다르게 설정
                json_path = osp.join(self.root_dir, 'classic_receipt/ufo/train.json')
            else:
                json_path = osp.join(self.root_dir, '{}_receipt/ufo/{}.json'.format(nation, split))
    ...
    def _infer_dir(self, fname):
        if 'classic' in self._lang_list and fname.startswith('0'):
            lang = 'classic'
